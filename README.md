...
1. [ ] 先做文档结构解析，按文档结构做粗拆分，再用递归字符分块兜底，保证大的主题单元不被拆分（2000-3000token）；再用语义分块做精细化拆分，控制单块大小（512-1024token）
2. [ ] 代码块、表格、图片特殊处理

> **已完成**：~~ParentDocument/Auto-merging Retriever 策略~~ — 两段自动合并（L3→L2→L1）已在 `rag_utils.py` 实现。

#### 召回层

1. [ ] BM25 的 k1 和 b 新增参数扫描（目前 `embedding.py` 中 k1=1.5、b=0.75 硬编码）
2. [ ] RRF 额外做 BM25 和 dense 的权重，可以通过 AB test 确定（目前使用 Milvus 内置 RRFRanker(k=60)，无额外权重参数）
3. [ ] 做一个小型标注集比较 dense only、sparse only、hybrid、hybrid + rerank 的 gold chunk

#### 生成层

1. [ ] 子问题分解（CoT、专门的分解小模型、判断分几个子问题）
2. [ ] 多文档 Refine（一次拼接、串行 Refine）
3. [ ] 多文档冲突处理（A 文档说 X，B 文档说非 X），回答中显式输出”来源存在冲突”
...
3. [ ] 新增问题类型判断，简单问题跳过复杂处理流程（目前所有知识类问题都走完整 RAG 图）
4. [ ] 扩展网络搜索能力
5. [ ] 支持多步骤规划与任务并行执行
6. [ ] 搭建路由器节点，由 LLM 自主判断下一步动作（目前仅在查询重写策略 step_back/hyde/complex 间路由）
7. [ ] 优化 memory 管理：集成 MemO、LangMem 等方案（目前仅 `summarize_old_messages` 截断摘要）
8. [ ] multi-agent：工具过多，把工具拆分给职责明确的专业化 agent
9. [ ] 历史记录会话名称可修改（`ChatSession` 模型无 name/title 字段）
10. [ ] 死循环检测与恢复：_is_stuck + attempt_loop_recovery（目前仅 `recursion_limit=8` 硬限制）

> **已完成**：~~暂停功能与人工介入机制~~ — 前端 `AbortController` + 后端 `GeneratorExit` 中显式 `agent_task.cancel()`。
